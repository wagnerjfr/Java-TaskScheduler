...
public class TaskFactory {
    ...
    public static Task create(String info) throws InstantiationException, IllegalAccessException,
    ...
    private static Task createTask(String className) throws InstantiationException, IllegalAccessException,
        ClassNotFoundException {
     ...
     */
    public static void testTasksClassNames(String[] tasks) throws NoClassDefFoundError, InstantiationException,
        ...
        for (String task_info : tasks) {
            ...
            if (task_info.contains(DEPENDENCY_SIGNAL) && task_info.contains(PARALLEL_SIGNAL))
                throw new TaskException(task_info + "\nYou can't have a parallel '--'"
                        + "and a sequential '->' dependencies in a main task.");

            task_info = task_info.replace(TASK_MAIN_SEPARATOR, DEPENDENCY_SIGNAL)
                    .replace(PARALLEL_SIGNAL, DEPENDENCY_SIGNAL);
